...
public class SpigotNexusAPI extends NexusAPI {
    ...
    public SpigotNexusAPI(NexusCore plugin) {
        super(plugin.getLogger(), new DataManager(), new TagManager(), new SpigotPlayerManager(plugin), new SpigotThreadFactory(plugin), new SpigotPlayerFactory(), new SpigotServerManager(plugin));
        this.plugin = plugin;
    }
