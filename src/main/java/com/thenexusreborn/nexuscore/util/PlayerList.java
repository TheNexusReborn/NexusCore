...
@SuppressWarnings({"JavadocDeclaration", "OptionalGetWithoutIsPresent", "MismatchedReadAndWriteOfArray", "DuplicatedCode", "DeprecatedIsStillUsed"})
public class PlayerList {
    ...
    public static PlayerList getPlayerList(Player player) {
        return getPlayerList(player, SIZE_DEFAULT);
    }
    
    public static PlayerList getPlayerList(Player player, int size) {
        if (!lookUpTable.containsKey(player.getUniqueId())) {
            return new PlayerList(player, size);
        }
        ...
        if (ReflectionUtil.getInstanceField(packet, "b") instanceof List) {
            ...
            for (Object player : players) {
                try {
                    ...
                    sendOLDTabPackets(getPlayer(), packetLoop, ((Player) player).getName(), false);
                } catch (Exception e) {
                    error();
                    e.printStackTrace();
                }
    ...
    private void addValue(int id, String name, boolean shouldUseSkin) {
        UUID uuid = (!name.isEmpty() && Bukkit.getOfflinePlayer(name).hasPlayedBefore())
    ...
    private static int getIDFromName(String id) {
        ...
        for (int i = 0; i < a.length; i++) {
            if (a[i].equalsIgnoreCase(id.charAt(indexAdder) + "")) {
                total = size1 * i;
                break;
            }
        }
...
class MojangAPIUtil {
    ...
    static {
        ...
        try {
            API_STATUS_URL = URI.create("https://status.mojang.com/check").toURL();
            GET_UUID_URL = URI.create("https://api.mojang.com/profiles/minecraft").toURL();
    ...
    public static void getUUIDAtTimeAsyncWithCallBack(String username, long timeStamp,
        ...
        try {
            URL url = URI.create("https://api.mojang.com/users/profiles/minecraft/" + username
                    + (timeStamp != -1 ? "?at=" + timeStamp : "")).toURL();
    ...
    public static class UUIDAtTime {
        ...
        public boolean equals(Object obj) {
            ...
            if (!(obj instanceof UUIDAtTime uuidAtTime)) {
                return false;
            }
    ...
    public static void getNameHistoryAsyncWithCallBack(UUID uuid, ResultCallBack<Map<String, Long>> callBack) {
        ...
        Validate.notNull(uuid, "uuid cannot be null!");
        try {
            URL url = URI.create("https://api.mojang.com/user/profiles/" + uuid.toString().replace("-", "") + "/names").toURL();
            makeAsyncGetRequest(url, (successful, response, exception, responseCode) -> {
                if (callBack == null) {
                    return;
                }
    ...
    public static Result<Map<String, Profile>> getUUID(List<String> usernames) {
        ...
        usernameJson.addAll(usernames.stream().filter(s -> !Strings.isNullOrEmpty(s)).toList());
    ...
    public static Result<SkinData> getSkinData(UUID uuid) {
        ...
        URL url;
        try {
            url = URI.create("https://sessionserver.mojang.com/session/minecraft/profile/"
                    + uuid.toString().replace("-", "") + "?unsigned=false").toURL();
        } catch (MalformedURLException e) {
            return new Result<>(null, false, e);
        }
    ...
    public static class SkinData {
        ...
        public boolean equals(Object obj) {
            ...
            if (!(obj instanceof SkinData skinData)) {
                return false;
            }
            return this.uuid.equals(skinData.uuid) && this.name.equals(skinData.name)
                    ...
                    && (Objects.equals(this.signedBase64, skinData.signedBase64));
