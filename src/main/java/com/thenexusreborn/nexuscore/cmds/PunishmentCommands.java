...
public class PunishmentCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().toLowerCase().contains("temp") || cmd.getName().equalsIgnoreCase("tb") || cmd.getName().equalsIgnoreCase("tm")) {
            if (!(args.length > 2)) {
                sender.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "/" + label + " <target> <length> <reason>"));
                return true;
            }
            ...
            try {
                ...
                sender.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "Invalid length argument."));
                return true;
            }
            ...
            if (!(args.length > 1)) {
                sender.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "/" + label + " <target> <reason>"));
                return true;
            }
        }
        ...
        if (targetRank.ordinal() < actorRank.ordinal()) {
            sender.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "You cannot " + type.name().toLowerCase() + " that player because their rank is higher than your own."));
            return true;
        }
        ...
        if (punishment.getType() == PunishmentType.MUTE) {
            if (targetPlayer != null && punishment.isActive()) {
                if (punishment.getType() == PunishmentType.MUTE) {
                    targetPlayer.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "You have been muted by " + punishment.getActorNameCache() + " for " + punishment.getReason() + ". (" + punishment.formatTimeLeft() + ")"));
                }
            }
            ...
            targetPlayer.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "You have been warned by " + punishment.getActorNameCache() + " for " + punishment.getReason() + "."));
            targetPlayer.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "You must type the code " + punishment.getAcknowledgeInfo().getCode() + " in chat before you can speak again."));
            ...
            if (punishment.getType() == PunishmentType.BLACKLIST) {
                if (punishment.isActive()) {
                    ...
                    String kickMessage = ColorHandler.getInstance().color(punishment.formatKick());
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                sender.sendMessage(ColorHandler.getInstance().color(MsgType.WARN + "Could not create the punishment. Please report to Firestar311."));
                e.printStackTrace();
                return;
            }
